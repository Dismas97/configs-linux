...
Debería proporcionar las herramientas básicas de una IDE al desarrollar en un lenguaje dado (y entorno), permitir tomar notas en general, crear documentos en general, proporcionar herramientas básicas para la documentación de software generar diagramas, etc y herramientas para la gestión organizacional y personal (agenda, todo).
...
#+begin_src emacs-lisp
  ...
  (show-paren-mode 1)
  ;;Emacs va a preferir abrir buffers temporales vertical
...
#+begin_src emacs-lisp 
  (setq org-capture-templates
      '(("t" "Tarea" entry (file+headline "~/Organizacion/Tareas.org" "TAREAS")
         "* TODO %?\n  %i\n  %a")
        ("d" "Diario" plain (file+olp+datetree "~/Organizacion/Diario.org")
...
#+begin_src emacs-lisp
    ...
    (defun fran/ejecutar-daemon (process-var process-call &optional ruta)
      (let ((proceso (symbol-value process-var)))
        (if (and proceso (process-live-p proceso))
            (progn
              (delete-process proceso)
              (set process-var nil)
              (message "Daemon detenido"))
          (let*
              ((dir-objetivo (cond
                               (ruta (if (file-directory-p ruta) ruta (file-name-directory ruta)))
                               (t (file-name-directory (or buffer-file-name default-directory)))))
               
               (default-directory dir-objetivo)
               (nuevo-proceso (if ruta
                     (funcall process-call ruta)
                   (funcall process-call))))
            (set process-var nuevo-proceso)
            (message "%s iniciado en: %s" (process-name nuevo-proceso) (cond (ruta ruta) (t dir-objetivo)))))))

    (defvar fran/live-server-p nil)
    (defun fran/live-server (&optional ruta)  
      (interactive (list (when current-prefix-arg (read-directory-name "Ejecutar en: "))))
      (fran/ejecutar-daemon 'fran/live-server-p (lambda () (start-process "live-server" "*live-server*" "live-server")) ruta))

    (defvar fran/vite-server-p nil)
    (defun fran/vite-server (&optional ruta)
      (interactive (list (when current-prefix-arg (read-directory-name "Ejecutar en: "))))
      (fran/ejecutar-daemon 'fran/vite-server-p (lambda () (start-process-shell-command "vite-server" "*vite-server*" "npx vite")) ruta))

    (defvar fran/php-server-p nil)
    (defun fran/php-server (&optional ruta)
      (interactive (list (when current-prefix-arg (read-file-name "Ejecutar en: "))))
      (fran/ejecutar-daemon 'fran/php-server-p
                            (lambda (&optional obj) (start-process-shell-command "php-server" "*php-server*" (concat "php -S localhost:8000 " obj))) ruta))
    ...
    (global-set-key (kbd "C-M-<return>") 'fran/nueva-linea-arriba)
...
#+begin_src emacs-lisp
  ...
  (keymap-set vertico-map "S-<left>" #'vertico-directory-up)
  (keymap-set vertico-map "S-<right>" #'vertico-insert)
#+end_src
...
Previzualizar en ventana actual
#+begin_src emacs-lisp 
...
#+begin_src emacs-lisp
  (use-package which-key :ensure t :init (which-key-mode) (which-key-setup-side-window-bottom))
...
Modos lenguajes js, golang, php etc.
#+begin_src emacs-lisp 
  ...
  (use-package css-mode :ensure t :mode "\\.css\\'")
...
#+begin_src emacs-lisp
  (use-package treesit-auto
    ...
    :config      
    (setq treesit-auto-langs '(c java javascript html css go rust php))
    (treesit-auto-add-to-auto-mode-alist '(c java javascript html css go rust php))
...
#+begin_src emacs-lisp
  ...
  (use-package eglot
    ...
    :config
    (add-to-list 'eglot-server-programs '((php-mode web-mode) . ("intelephense" "--stdio")))
    :custom
...
#+end_src
Transclusion, embebe contenido de links al archivo org
#+begin_src emacs-lisp
(use-package org-transclusion :ensure t)
#+end_src
** GUI
#+begin_src emacs-lisp
  (setq fran/tema-actual 'modus-operandi-tinted)
